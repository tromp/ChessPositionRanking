...
|---------|-------------|-----------|---------|----------|
| ![png](img/2389704906374985477664262349386869232706664089.png)  | ![png](img/4181480207421137985661966414043624022270490788.png)  | ![png](img/6496802122365460857591560979886251701655591631.png)  | ![png](img/8407831187507152187492557297490243416405059949.png)  | ![png](img/8545434809537180765086507091416427802529352632.png) |

The first 2 have White to move, and the other 3 have Black to move.

# These 5 positions have two things in common

1. They're all legal. That is, reachable from the starting position in a valid game of chess.

2. They're randomly generated.  
In fact, these 5 are all the legal positions among a sample generated by picking 100 random integers in a specific range, and converting each integer into a corresponding position.
...
The 5 legal positions all have a multiplicity of 1, while the average multiplicity of all 100 positions is 1.04.
With a 95% [confidence level](https://en.wikipedia.org/wiki/Binomial_proportion_confidence_interval#Normal_approximation_interval), this yields an estimated number of legal positions of (5% +- 1.96 sqrt(5% * 95% / 100)) * N / 1, or (4.4+-3.7)x10^44,
which is far less than one digit of accuracy, and just gives us the order of magnitude.
To obtain a full digit of accuracy, we need to reduce the error by a factor of 10, which requires a 100x bigger sample. File "testRnd10kResearch" contains a sample of 10,000 random positions subjected to a relatively simple legality check, that finds all but 919 positions to be illegal for trivial reasons.
