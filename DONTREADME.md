...
but if we extend each object with its multiplicity n and an index in the range 0..n-1, making the objects uniquely rankable, then we do obtain a 1-1 correspondence.

This Haskell based software suite implements a ranking of uniquely rankable chess positions (urpositions) of size N = 8726713169886222032347729969256422370854716254.
It uses Forsyth–Edwards Notation or FEN [0] for input and output, with the following modifications:
 * we consider a position to feature en-passant only when the pawn that just moved 2 spaces finds itself adjacent to an opponent pawn.
 * we use the last two fields for unique rankability
...
With a 95% confidence level [1], this yields an **estimated number of legal positions of** (5.2% +- 1.96 sqrt(5.2% * 94.8% / 1000)) * N / 1.077, or **4.2e44 +- 1.1e44**.

The file sortedRnd1kResearchManual contains all 1000 positions together with their legality determination.
...
We checked invertibility of the ranking by ranking the results (Makefile targets testRnd1kRanking, testRnd10kRanking, and testRnd100kRanking), and comparing with the origin integers used for unranking.

We unranked these to obtain random urpositions (Makefile targets testRnd1kFENs,
testRnd10kFENs, and testRnd100kFENs), and ran some legality tests that
found 90+% of the positions to be definitely illegal (Makefile targets
testRnd1kResearch, testRnd10kResearch, testRnd100kResearch).
